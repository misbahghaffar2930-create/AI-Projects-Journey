...
delivery_info = "3-5 working days"
contact_info = "WhatsApp: 0300-1234567"
location = "Lahore"

print(f"--- Welcome to {shop_name} Chatbot ---")
...
while True:
    ...
    if "exit" in user_input:
        print(f"{shop_name}: Allah Hafiz! Visit again.")
        break

    # 1. Product and Price Check
    elif "price" in user_input or "available" in user_input or "dresses" in user_input:
        print(f"{shop_name}: Humare paas ye items available hain:")
        for item, price in products.items():
            print(f"- {item.capitalize()}: {price}")

    # 2. Delivery Check
    elif "delivery" in user_input or "days" in user_input or "time" in user_input:
        print(f"{shop_name}: Delivery mein {delivery_info} lagtay hain.")

    # 3. Location and Contact
    elif "location" in user_input or "address" in user_input:
        print(f"{shop_name}: Hum {location} mein hain. {contact_info}")
    
    elif "whatsapp" in user_input or "contact" in user_input:
        print(f"{shop_name}: Aap humein yahan contact kr skte hain: {contact_info}")

    # 4. Out of Scope Rule (Sirf shop ki baat)
    else:
        print(f"{shop_name}: Maaf kijiyega, main sirf StyleZone PK aur uski products k bare mein bta sakta hoon.")
